...
class SignUpAPIView(APIView):
    ...
    @extend_schema(
        request=SignUpSerializer, responses={200: ObtainTokenResponseSerializer}
    )
    def post(self, request):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                token, refresh_token, payload = jwt_service.encode(
                    AccountSerializer(instance=account).data
                )
                response_serializer = ObtainTokenResponseSerializer(
                    {"token": token, "refresh_token": refresh_token, "payload": payload}
                )
...
class SignInAPIView(APIView):
    ...
    @extend_schema(
        request=SignInSerializer, responses={200: ObtainTokenResponseSerializer}
    )
    def post(self, request):
        data = request.data
        try:
            ...
            return JsonResponse(
                {"message": str(ACCOUNT_EXCEPTION.ACCOUNT_NOT_EXIST)},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            return JsonResponse(

...
class ElizaBotDetailAPI(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            return JsonResponse(
                ElizaBotResponseSerializer(instance=eliza_bot).data,
                status=status.HTTP_200_OK,
            )
        except ElizaBot.DoesNotExist:
            return JsonResponse(
                {"message": ELIZA_BOT_EXCEPTION.ELIZA_BOT_NOT_EXIST},
    ...
    @extend_schema(
        request=ElizaBotSerializer,
        responses={200: ElizaBotResponseSerializer},
        tags=["eliza-bot"],
    )
    def put(self, request, id):
        try:
            ...
            serializer = ElizaBotSerializer(
                instance=eliza_bot, data=request.data, partial=True
            )
            if serializer.is_valid():
                updated_eliza_bot = serializer.save()
                return JsonResponse(
                    ElizaBotResponseSerializer(instance=updated_eliza_bot).data,
                    status=status.HTTP_200_OK,
                )
        except ElizaBot.DoesNotExist:
            return JsonResponse(
    ...
    def delete(self, request, id):
        try:
            ...
            eliza_bot.delete()
            return JsonResponse(
                NotificationSerializer({"message": "success"}).data,
                status=status.HTTP_200_OK,
            )
        except ElizaBot.DoesNotExist:
            return JsonResponse(
                {"message": str(ELIZA_BOT_EXCEPTION.ELIZA_BOT_NOT_EXIST)},
...
class ElizaBotAPIView(APIView):
    ...
    @extend_schema(
        responses={200: ElizaBotResponseSerializer(many=True)}, tags=["eliza-bot"]
    )
    def get(self, request):
        try:
            eliza_bots = ElizaBot.objects.filter(owner=request.user)
            return JsonResponse(
                ElizaBotResponseSerializer(eliza_bots, many=True).data,
                status=status.HTTP_200_OK,
                safe=False,
            )
        except Exception as e:
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                return JsonResponse(
                    ElizaBotResponseSerializer(eliza_bot).data,
                    status=status.HTTP_200_OK,
                )
            else:
                return JsonResponse(
...
class ElizaBotGenerateResponseAPIView(APIView):
    ...
    @extend_schema(
        request=ElizaBotInputMessageSerializer,
        responses={200: ElizaBotGenerateResponseSerializer},
        tags=["eliza-bot"],
    )
    def post(self, request, id):
        try:
            ...
            if serializer.is_valid():
                ...
                response = eliza_service.response(
                    text=serializer.validated_data["message"]
                )
                response_serializer = ElizaBotGenerateResponseSerializer(

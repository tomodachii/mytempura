...
class ReasmbAdmin(admin.ModelAdmin):
    ...
    search_fields = ['decomp__pattern', 'template', 'decomp__keyword__word', 'decomp__keyword__bot__name']
    list_display_links = ['id', 'template']

    def bot_link(self, obj):
        url = reverse('admin:keywordrecognition_elizabot_change', args=[obj.decomp.keyword.bot.pk])
        return format_html('<a href="{}">{}</a>', url, obj.decomp.keyword.bot)
    bot_link.short_description = 'bot'

    def keyword_link(self, obj):
        url = reverse('admin:keywordrecognition_keyword_change', args=[obj.decomp.keyword.pk])
